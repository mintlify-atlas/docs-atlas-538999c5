---
title: Relationships
description: Work with database relationships including has-many, belongs-to, and many-to-many associations in IHP
---
...
- user_id # Reference to the `User` that is the author.
...
fetchPostWithRecords postId = do
    ...
    -- This field is a query builder with the right `WHERE` condition already applied (referencing the Post ID),
...
showComment comment commentUsers =
    ...
    where
        authorName = commentUsers
            ...
            -- Get the user's name. As `find` returns a Maybe value we need to use `maybe ""`
            |> maybe "" (get #name)
```
