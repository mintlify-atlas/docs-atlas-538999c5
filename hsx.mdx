---
title: HSX Templating
description: JSX-like syntax for type-safe HTML templating in Haskell
---
...
**If the variable is any other custom Haskell data structure**: it will first be converted to a string representation by calling `show` on it. You can add a custom `ToHtml` instance to customize rendering a data structure.
...
<Tabs>
  ...
  <Tab title="False">
    ...
    This will not render the attribute, as specified in the HTML standard:
...
When set to `False`, the output HTML will keep the attribute and set it to `"false"`:
...
This will just literally output the string `{myHaskellExpr}` without evaluating the Haskell expression itself. This is because JavaScript usually uses `{}` for object expressions like `{ a: "hello" }`.
...
We also support a "no render" comments, that can be used for adding comments inside `hsx`, but without being rendered to HTML:
...
The underlying HTML library blaze currently does not support an empty HTML attribute. Therefore empty attributes are implemented by setting the attribute value to the attribute name. This is valid HTML supported by all browsers. Therefore the generated HTML looks like this:
...
When dealing with `Maybe` values you sometimes want to conditionally render something. In HSX you usually write it like this:
...
What about if the country is a empty string? It's better to deal with the problem at create time of the `User` record already. Use `emptyValueToNothing` to transform an empty `Just ""` to `Nothing` before inserting it into the db:
...
In HSX all variables and expressions are automatically escaped to avoid XSS.
...
Sometimes you want to insert some actual HTML code to your HSX using a variable. E.g. when your app is rendering markdown, the generated HTML by the markdown library should not be escaped.
...
You might want to have this `&` character not escaped. E.g. to use Tailwind Arbitrary Variants. In that case wrap the attribute value with a call to `preEscapedTextValue`:
...
<Steps>
  <Step title="Create a custom HSX module">
    First, create a new module for your custom HSX (e.g., `Application.Helper.CustomHsx`):
  ...
  <Step title="Make it available in your views">
    Add it to your view helpers module:
  ...
  <Step title="Use it in your views">
    ```haskell
    [myHsx|
